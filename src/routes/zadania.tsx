...
function QuestsPage() {
  ...
  return <GamePage title="Zadania" subtitle={`Dzienny zestaw na ${state.date} według czasu polskiego.`}><div className="space-y-8">
    ...
    <section><h2 className="flex items-center gap-2 font-display text-2xl"><BookOpen className="h-5 w-5" /> Badania Profesora Oaka</h2><div className="glass-panel mt-4 grid gap-5 rounded-2xl p-5 sm:grid-cols-[96px_1fr]"><img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/trainers/old-man.png" alt="Profesor Oak" className="h-24 w-24 [image-rendering:pixelated]" /><div>{state.research ? <><p className="text-sm">„{state.research.status === "completed" ? state.research.dialog_complete : state.research.dialog_intro}”</p><p className="mt-3 font-medium">Postęp: {state.research.progress}/{state.research.target}</p>{state.research.status === "completed" ? <Button className="mt-3" disabled={busy} onClick={() => void run(() => claimOak(), "Badanie ukończone, nagroda odebrana!")}>Odbierz nagrodę</Button> : null}</> : nextOak ? <><p className="text-sm">„{nextOak.intro}”</p><p className="mt-2 text-xs text-muted-foreground">Wymagany poziom trenera: {nextOak.level}</p><Button className="mt-3" disabled={busy || state.trainer_level < nextOak.level} onClick={() => void run(() => startOak(), "Profesor Oak rozpoczął nowe badanie.")}>Rozpocznij badanie</Button></> : <p>Ukończyłeś wszystkie dostępne badania.</p>}</div></div></section>
  </div></GamePage>;
}