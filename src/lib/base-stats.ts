/**
 * Bazowe statystyki gatunków (HP, Atak, Obrona, Atak Spec., Obrona Spec., Szybkość)
 * oraz lista Pokémonów legendarnych i mitycznych. Dane wygenerowane z PokéAPI.
 */
...
const DEFAULT_BASE: BaseStats = [60, 60, 60, 60, 60, 60];

/** Bazowe statystyki gatunku; nieznane gatunki dostają średnie wartości. */
export function baseStats(speciesId: number | null | undefined): BaseStats {
  if (!speciesId) return DEFAULT_BASE;
  return BASE_STATS[speciesId] ?? DEFAULT_BASE;
}

/** Suma bazowych statystyk — miara ogólnej siły gatunku. */
export function baseStatTotal(speciesId: number | null | undefined): number {
  return baseStats(speciesId).reduce((sum, value) => sum + value, 0);
}
...
const LEGENDARY_IDS = new Set<number>([
  ...
  809, 888, 889, 890, 891, 892, 893, 894, 895, 896, 897, 898, 905, 1001, 1002, 1003, 1004, 1007, 1008, 1014, 1015, 1016,
  1017, 1024, 1025,
]);

/** Legendarne i mityczne gatunki — nie pojawiają się w dzikich spotkaniach. */
export function isLegendary(speciesId: number | null | undefined): boolean {
  return !!speciesId && LEGENDARY_IDS.has(speciesId);
}
/**
 * Oblicza maksymalne HP Pokémona zgodnie z klasycznym wzorem.
 * EV (punkty treningu) wynoszą domyślnie 0 przy złapaniu.
 */
export function calculateMaxHP(base: number, iv: number, ev: number, level: number): number {
  const coreStat = Math.floor(0.01 * (2 * base + iv + Math.floor(0.25 * ev)) * level);
  return coreStat + level + 10;
}

/**
 * Oblicza pozostałe statystyki (Atak, Obrona, itd.) w razie potrzeby.
 */
export function calculateStat(
  ...
  const coreStat = Math.floor(0.01 * (2 * base + iv + Math.floor(0.25 * ev)) * level);
  return Math.floor((coreStat + 5) * natureMultiplier);
}
